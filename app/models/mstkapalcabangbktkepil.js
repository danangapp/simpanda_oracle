...
MstKapalCabangBktKepil.create = async(newMstKapalCabangBktKepil, result) => {
		newMstKapalCabangBktKepil.id = "mst_kapal_cabang_bkt_kepil_seq.nextval"
		const hv = f.headerValue(newMstKapalCabangBktKepil);
		var queryText = "INSERT INTO \"mst_kapal_cabang_bkt_kepil\" " + hv + " RETURN \"id\" INTO :id";
		const exec = f.query(queryText, 1);
		delete newMstKapalCabangBktKepil.id;
		const res = await exec;

		result(null, { id: res.outBinds.id[0], ...newMstKapalCabangBktKepil });
};
...
MstKapalCabangBktKepil.getAll = async (param, result) => {
...
MstKapalCabangBktKepil.updateById = async(id, mstkapalcabangbktkepil, result) => {
