...
PanduSchedule.create = async (newPanduSchedule, result) => {
	for (var a in newPanduSchedule) {
		...
		const hv = f.headerValue(newPanduSchedule);
...
PanduSchedule.findById = async (id, result) => {
	const resQuery = await f.query("SELECT * FROM \"pandu_jaga\" WHERE \"pandu_schedule_id\" = '" + id + "'");
			...
			for (var b in c) {
				if (b === "pandu_jaga") {
					...
					for (var d in e) {
						...
						await f.query("INSERT INTO \"pandu_jaga\" (" + header1 + ") values (" + value1 + ")", 2);
					}
				}
			}
		}
